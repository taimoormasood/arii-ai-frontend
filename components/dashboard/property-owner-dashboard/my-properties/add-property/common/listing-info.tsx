...
import { timeOptions } from "../property-types/common/constants";
...
export const listingInfoSchema = z.object({
  ...
  totalBedrooms: z.coerce
    .number({
      required_error: "Total bedrooms is required.",
      invalid_type_error: "Only numeric digits are allowed.",
    })
    .int("Bedrooms must be a whole number.")
    .min(1, "Bedrooms must be between 1 and 10.")
    .max(10, "Bedrooms must be between 1 and 10."),
  totalBathrooms: z.coerce
    .number({
      required_error: "Total bathrooms is required.",
      invalid_type_error: "Only numeric digits are allowed.",
    })
    .int("Total bathrooms must be a whole number.")
    .min(1)
    .max(10, "Bathrooms must be between 1 and 10."),
  squareFootage: z
    .string()
    .optional()
    .refine(
  ...
  // Alternative: More explicit File array type
  propertyPhotos: z
    .array(
      z.union([
        z
    ...
    .min(1, "At least one property photo is required."),
});
...
const ListingInfo = () => {
  ...
  // Get existing listing details data
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      totalBedrooms: listingDetailsData?.totalBedrooms
        ? Number(listingDetailsData.totalBedrooms)
        : 0,
      totalBathrooms: listingDetailsData?.totalBathrooms
        ? Number(listingDetailsData.totalBathrooms)
        : 0,
      squareFootage: listingDetailsData?.squareFootage || "",
      ...
      // customPets: listingDetailsData?.customPets || [],
  ...
  const onSubmit = async (isSaveExit: boolean, data: FormValues) => {
    ...
    const propertyId = isEditMode ? editingPropertyId : propertyInfoData?.id;
    ...
    const payload = {
      ...
      total_bedrooms: String(data.totalBedrooms),
      total_bathrooms: String(data.totalBathrooms),
      square_footage: data.squareFootage || "",
      description: data.description,
      pets_allowed: data.petsAllowed,
      pet_types: allPetTypes,
      other_pets: data.customPets,
      availability_duration: data.availability_duration,
      showing_availability: showingAvailability,
      photo: data.propertyPhotos.filter((file) => file instanceof File),
      page_saved: 2,
    };
    ...
    const mutationOptions = {
      onSuccess: () => {
        if (isSaveExit) {
          ...
          // Save to store using semantic key
          // Separate Files from URLs
          const filePhotos = data.propertyPhotos.filter(
            (item): item is File => item instanceof File
          );
          const urlPhotos = data.propertyPhotos.filter(
            (item): item is string => typeof item === "string"
          );

          updateStepData("listingDetails", {
            listedBy: data.listedBy,
            totalBedrooms: String(data.totalBedrooms),
            totalBathrooms: String(data.totalBathrooms),
            squareFootage: data.squareFootage || "",
            description: data.description,
            petsAllowed: data.petsAllowed,
            petTypes: data.petTypes,
            other_pets: data.customPets, // store custom pets as other_pets
            customTags: data.customTags,
            availability: data.availability,
            availability_duration: data.availability_duration,
            propertyPhotos: filePhotos,
            existingPhotoUrls: urlPhotos.length > 0 ? urlPhotos : undefined,
          });
    ...
    if (isEditMode && editingPropertyId) {
      updateListingInfo(
        { ...payload, propertyId: editingPropertyId },
        mutationOptions
      );
    } else {
    ...
    <div className="bg-white rounded-lg p-6 shadow-sm mx-auto w-full">
      ...
      <Form {...form}>
        ...
        >
          ...
          <CustomTextarea
            name="description"
            label="Room and Shared Space Description"
            required
            placeholder="Describe the room and shared amenities..."
            rows={4}
          ...
          <div className="flex gap-4 pt-6 items-end justify-end">
            <div className="flex gap-4">
              <Button
                type="button"
                variant="outline"
                onClick={() => onSubmit(true, form.getValues())}
