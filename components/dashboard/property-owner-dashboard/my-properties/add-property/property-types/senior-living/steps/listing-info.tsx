...
export const listingInfoSchema = z.object({
  ...
  totalUnits: z.coerce
    .number({ required_error: "Total number of units is required." })
    .min(1, "Minimum 1 units required.")
  ...
  careServices: z.array(z.string()).optional(),
...
const careServices = [
  { id: "on_site_nurse", name: "On-site Nurse" },
  { id: "emergency_call_system", name: "Emergency Call System" },
  { id: "medication_management", name: "Medication Management" },
  { id: "care_staff", name: "24/7 Care Staff" },
  { id: "daily_willness_checks", name: "Daily Wellness Checks" },
  { id: "mobility_assistance", name: "Mobility Assistance" },
  { id: "housekeeping_services", name: "Housekeeping Services" },
  { id: "meal_services", name: "Meal Services" },
  { id: "physical_therapy", name: "Physical Therapy Room" },
];
...
const ListingInfo = () => {
  ...
  useEffect(() => {
    if (apiListingDetails && propertyId) {
      ...
      const listingStoreData = {
        ...
        totalUnits: apiListingDetails.number_of_units
        ...
        careServices: apiListingDetails.care_services || [],
      ...
      form.reset({
        ...
        totalUnits: Number(listingStoreData.totalUnits),
        ...
        careServices: listingStoreData.careServices,
    ...
    if (listingDetailsData && !isLoadingPropertyDetail) {
      ...
      if (!hasExistingListingData) {
        form.reset({
          ...
          totalUnits: listingDetailsData.totalUnits
            ? Number(listingDetailsData.totalUnits)
            : 0,
          ...
          careServices: listingDetailsData.careServices || [],
  ...
  const onSubmit = async (isSaveExit: boolean, data: FormValues) => {
    ...
    const payload: any = {
      ...
      number_of_units: String(data.totalUnits),
    ...
    const mutationOptions = {
      onSuccess: (response: any) => {
        ...
        const updatedStoreData = {
          ...
          description: data.description,
          careServices: data.careServices,
  ...
  const handleCareServiceToggle = (service: string) => {
    const currentServices = form.getValues("careServices") || [];
    const isSelected = currentServices.includes(service);

    if (isSelected) {
      form.setValue(
        "careServices",
        currentServices.filter((type) => type !== service)
      );
    } else {
      form.setValue("careServices", [...currentServices, service]);
    }
  };
          ...
          {careServices.length > 0 && (
            <div className="flex flex-wrap gap-3">
              ...
              <div className="flex flex-wrap gap-3 w-full">
                {careServices?.map((service: any) => {
                  const careServicesValue = form.watch("careServices");
                  const isSelected =
                    Array.isArray(careServicesValue) &&
                    careServicesValue.includes(service.id);
                  ...
                  return (
                    ...
                    >
                      <CustomCheckbox
                        ...
                        stopPropagation
                        onChange={(checked) => {
                          const current = form.getValues("careServices") || [];
                          if (checked) {
                            form.setValue(
                              "careServices",
                              [...current, service.id],
                      ...
                      />
                      <span className="text-xs flex-1">{service.name}</span>
                    </div>
                  );
                })}
              </div>
