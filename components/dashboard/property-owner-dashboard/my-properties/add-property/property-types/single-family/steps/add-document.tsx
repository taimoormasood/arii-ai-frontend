...
import { DocumentForm } from "../../common/document-form";
...
// Static document types for edit mode to ensure previously selected types are available
...
export default function AddDocument({ pageSaved = 7 }: AddDocumentProps) {
  ...
  const isPending = isAddingDocument;
    ...
    if (isEditMode && apiDocuments && apiDocuments.length > 0 && propertyId) {
      const transformedDocuments: UploadedDocument[] = apiDocuments.map(
        (doc: any) => ({
          ...
          documentId: doc.id, // Include the API document ID for existing documents
          ...
          isExpanded: false,
          existingDocumentUrl: doc.document,
          isExisting: true,
        })
      );
  ...
  }, [isEditMode, apiDocuments, propertyId, updateStepData]);

  // Load existing documents in edit mode from store (fallback)
  useEffect(() => {
  ...
  const handleSubmitDocument = async (isSaveExit: boolean) => {
    ...
    const currentPropertyId = isEditMode ? editingPropertyId : propertyId;
    ...
    const existingDocuments = documents.filter(
      (doc) => doc.file === null && doc.documentId !== undefined
    );
    ...
    const payload: any = {
      ...
      data: {
        ...
        existing_data: [], // Required by TypeScript type for data structure
      },
    };
    ...
    if (existingDocuments.length > 0) {
      payload.existing_data = {
        data: existingDocuments.map((doc) => ({
          ...
          title: doc.metadata.title,
          document_type: doc.metadata.type.toLowerCase().replace(/ /g, "_"),
          // Note: visibility is not included as documents can't be fully edited
        })),
      };
    }

    // Always use the CREATE endpoint - backend handles updates when ID is provided
    addDocument(payload, {
      onSuccess: async () => {
        try {
          const res = await publishProperty(propertyId!, { published: true });
    ...
    <div className="mt-6 p-2">
      <div className="space-y-2 p-2">
        ...
        {/* {availableDocumentTypes.length > 0 && (
          <div className="text-sm text-gray-500">
            {isEditMode
              ? `Document types: ${availableDocumentTypes.length} available`
      ...
      <div className="flex justify-between mt-6 w-full">
        <Button
          type="button"
          className="px-6 py-2  bg-white hover:bg-gray-100  text-gray-900 border border-gray-200 rounded-md font-semibold"
          onClick={() => {
        ...
        <div className="flex space-x-3">
          ...
          <Button
            className="px-6 py-2 bg-primary-600 text-white hover:bg-primary-700 rounded-md transition-colors"
