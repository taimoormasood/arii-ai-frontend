...
const unitSchema = z.object({
  ...
  unitPhotos: z
    ...
    .optional(), // Make optional for edit mode validation
});
...
const multiFamilyOptions = [
  ...
  { value: "rear_unit", label: "Rear Unit" },
  { value: "front_unit", label: "Front Unit" },
];
...
const seniorLivingOptions = [
  { value: "studio", label: "Studio" },
  { value: "1_bedroom", label: "1 Bedroom" },
  { value: "2_bedroom", label: "2 Bedroom" },
  { value: "companion_suite", label: "Companion Suite" },
  { value: "accessible_unit", label: "Accessible Unit" },
  { value: "memory_care_unit", label: "Memory Care Unit" },
  { value: "private_room", label: "Private Room" },
  { value: "shared_room", label: "Shared Room" },
];

const unitTypeMap: Record<string, { label: string; value: string }[]> = {
  student_housing: studentHousingOptions,
  senior_living: seniorLivingOptions,
  multi_family: multiFamilyOptions,
  apartment_unit: apartmentUnitOptions,
};
...
const UnitInfo = () => {
  ...
  const unitTypeOptions =
    propertyType && unitTypeMap[propertyType] ? unitTypeMap[propertyType] : [];
  const {
    getStepData,
    getUnitStepData,
  ...
  const form = useForm<UnitFormValues>({
    ...
    mode: "onChange",
    defaultValues: {
      unitNumber: unitInfoFormData?.number
        ? Number(unitInfoFormData.number)
        : undefined,
      unitType: unitInfoFormData?.type || "",
      floorNumber: unitInfoFormData?.floorNumber
        ? Number(unitInfoFormData.floorNumber)
        : undefined,
      unitSize: unitInfoFormData?.size
        ? Number(unitInfoFormData.size)
        : undefined,
      totalBedrooms: unitInfoFormData?.bedrooms
        ? Number(unitInfoFormData.bedrooms)
        : undefined,
      totalBathrooms: unitInfoFormData?.bathrooms
  ...
  useEffect(() => {
    if (currentUnitData && propertyId && unitId) {
      ...
      const unitStoreData = {
        ...
        bedrooms: unitDetail.bedrooms || 1,
        bathrooms: unitDetail.bathrooms || 1,
      ...
      form.reset({
        unitNumber: parseInt(unitDetail.number) || 0,
        unitType: unitDetail.type || "",
        floorNumber: unitDetail.floor_number
          ? parseInt(unitDetail.floor_number)
          : undefined,
        unitSize: unitDetail.size ? parseInt(unitDetail.size) : undefined,
        totalBedrooms: parseInt(unitDetail.bedrooms) || 1,
        totalBathrooms: parseInt(unitDetail.bathrooms) || 1,
        unitPhotos: [],
      });
  ...
  const onSubmit = async (isSaveExit: boolean, data: UnitFormValues) => {
    ...
    const payload: any = {
      property: propertyId,
      number: String(data.unitNumber),
      type: data.unitType,
      floor_number: data.floorNumber ? String(data.floorNumber) : undefined,
      size: data.unitSize ? String(data.unitSize) : undefined,
      bedrooms:
        data.totalBedrooms !== undefined
          ? String(data.totalBedrooms)
          : undefined,
      bathrooms:
        data.totalBathrooms !== undefined
          ? String(data.totalBathrooms)
          : undefined,
      page_saved: 1,
    };
