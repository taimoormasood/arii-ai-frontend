...
export default function UnitDocumentsTable() {
  ...
  const { data: documentsData, refetch } = useGetPropertyDocuments(
    Number(propertyId),
    unitId
  );
  ...
  const handleDownload = (title: string, fileUrl: string) => {
    ...
    link.href = fileUrl;
    link.download = title; // Optional: set a default file name
