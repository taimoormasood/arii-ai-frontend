...
export default function PropertyCard({
  ...
  unitData: PropertyCardProps["unitData"];
  actions?: PropertyCardProps["actions"];
}) {
  ...
  const { updateUnitStepData, setEditMode, setSelectedPropertyType } =
    usePropertyStore();
  ...
  const handleUnitEdit = () => {
    ...
    setEditMode(true, parseInt(id as string));

    // Update unit step data with unit ID
    updateUnitStepData("unitInfo", {
      id: unitData?.detail?.id,
    });

    // Navigate to unit edit page
    router.push(
      `/my-properties/add-unit/${propertyType}?unitId=${unitData?.detail?.id}`
    );
  };
      ...
      <div className="bg-white rounded-2xl shadow border border-gray-100 p-3 flex flex-col relative transition hover:shadow-md">
        ...
        <div className="relative w-full h-44 overflow-hidden">
          <Image
            ...
            priority
            src={
              unitData?.detail?.photos[0]?.photo ||
        ...
        <div className="flex-1 flex flex-col gap-1 px-1 pt-3 pb-2">
          ...
          {propertyType !== "university_housing" && (
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-gray-700 text-xs font-medium ml-1">
                {formatText(unitData?.detail?.type) || ""}
              </span>
            </div>
          )}
          ...
          <div className="flex items-center justify-between gap-2 mt-2">
            ...
            <span className="text-gray-700 text-xs font-medium ml-1">
              {formatText(unitData?.rental_details?.assigned_tenant) ||
                "No Tenant Assigned"}
            </span>
          </div>
          ...
          <div className="flex items-center justify-between gap-2 mt-2">
            ...
            <Badge className="bg-yellow-100 capitalize text-yellow-800 hover:bg-yellow-100 border-yellow-200 text-xs px-3 py-1 rounded-full">
              {unitData?.detail?.status || "Unknown"}
            </Badge>
          </div>
          ...
          <div className="flex items-center justify-between mt-4">
            <span className="text-xl font-bold text-gray-900">
              ${unitData?.rental_details?.rent || "N/A"}
            </span>
            <Link
              // href={`/my-properties/property-detail/${id}?propertyType=${propertyType}`}
              href={`/my-properties/property-unit-detail/${unitData?.detail?.id}?propertyType=${propertyType}&propertyId=${id}`}
